...
\vspace{10pt}
\begin{enumerate}
  \item[2] Variationally minimize the energy functional with respect to the $c$-coefficients 
  \begin{equation}
...
\onslide<2->{
  ...
  \end{equation}
  aka, {\it complete diagonalization}.
}
\end{enumerate}
\end{frame}
...
\begin{frame}{CC theory for excited state properties}
 ...
 \onslide<2->{
  ...
  \end{eqnarray}
  Here, $\hat{R}$ ($\hat{L}$) is a linear excitation (de-excitation) operator for the $m$-th excited state including up to $M$-electron excitations.\
 }
 \vspace{10pt}
 \onslide<3->{
...
\begin{frame}{Time-domain CC theory for linear absorption spectra}
 ...
 \begin{block}{}
  \begin{equation}
  \color{blue}{I(\omega) =  \int_{-\infty}^{\infty}  dt~ e^{-i \omega t}{\langle \tilde{M}_0|M(-t)\rangle}} 
  \end{equation}
 \end{block}
 }
\end{frame}
...
\begin{frame}{Time-domain CC theory for linear absorption spectra}
 TD-EOM-CC2 vs EOM-CC2 (Basis: cc-pVDZ)
...
\begin{frame}{Time-domain CC theory for linear absorption spectra}
 Extreme UV spectrum of the Neon isoelectronic series (Basis: aug-cc-pVQZ)
...
\onslide<3->{
 ...
 \footnotesize{D. G. A. Smith {\it et al.}, {\it JCTC}, {\bf 14}, 3504, (2018)}
 \footnotesize{github.com/liresearchgroup/chronusq\_public}
}
\end{frame}

\begin{frame}{Caveats of TD-CC methods}
\onslide<1->{Each time step in the TD-EOM-CC method corresponds to several sigma builds, 
which is expensive [scales with at least $\mathcal{O}(N^5)$ cost]! \\
\vspace{5pt}
}
\onslide<2->{
Recall that:
\begin{equation}
    I(\omega) = \int_{-\infty}^{\infty}  dt~ e^{-i \omega t}{\langle \tilde{M}_0|M(-t)\rangle}.
\end{equation}
}
\onslide<3->{
In practice, we reexpress the integral above as a sum of the form,
\begin{equation}
I(\omega) = \lim_{T/\Delta t \to \infty} \frac{\Delta t}{T} \sum_{k = 0}^{T/\Delta t} e^{-i \omega k \Delta t}{\langle \tilde{M}_0|M(-k \Delta t)\rangle}. 
\end{equation}
}
\onslide<4->{
By choosing a long enough propagation time $T$ and a small enough time-step 
$\Delta t$, we can arrive at a good approximation for $I(\omega)$.
}
\onslide<5->{
{\color{red} Can we minimize the amount of time steps required to yield 
an ``almost" converged spectrum?} 
}
\end{frame}
...
\begin{center}
 ...
 \onslide<3->{
            \node[align=center,blue,font={\small}] at (1.6in,2.2in) {Even higher sampling rates are required to \\ fully recover the oscillator strength!};
 }
  \end{tikzpicture} 
\end{center}
\end{frame}
...
\begin{center}
 \begin{tikzpicture}
 \onslide<1->{
            \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[scale=0.8]{figures/nyquist5.eps}};
 }
 \onslide<2->{
            \node[align=center,red,font={\small}] at (1.6in,2.2in) {So, the time-step is out of our hands!};
 }
  \end{tikzpicture}
\end{center}
\end{frame}
...
\begin{eqnarray}
 c_k &\equiv& \langle  \tilde{M}_0|M(k\Delta t)\rangle \nonumber \\
